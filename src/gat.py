...
class GAT(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        x2 = F.dropout(x, self.dropout, training=self.training)   # dropout，防止过拟合
        x2 = F.elu(self.out_att(x2, adj))   # 输出并激活
        return F.log_softmax(x2, dim=1), x2  # log_softmax速度变快，保持数值稳定